...
import { getKebiao, getTransactions, loginToMagipoke } from '@/api/magipoke.server'
...
export interface SimplifyCourse {
  name: string
  begin: string
  end: string
  location: string
...
export async function GET() {
  ...
  for (const transaction of transactionData.data) {
    // 周筛，用课表的nowWeek
    if (transaction.title.includes('private') || transaction.title.includes('私有'))
      continue
    if (transaction.content.includes('private') || transaction.content.includes('私有'))
      continue
