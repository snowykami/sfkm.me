...
export function deriveLyricThemeColors(themeColor: string) {
    ...
    const dayText = hslToHex(h, Math.min(1, s * 1.2), Math.max(0.7, l * 0.7));
    const otherDayText = hslToHex(h, Math.min(0.3, s * 0.3), Math.max(0.3, l * 0.3));
    const dayBg = hslToHex(h, s * 0.15, 0.4) + "80"; // 0.94透明度
    const dayProgress = hslToHex(h, Math.min(0.8, s * 0.8), Math.max(0.6, l * 0.6));
    
    const nightText = hslToHex(h, Math.min(1, s * 1.2), Math.max(0.7, l * 0.7));
    ...
    return { dayText, dayBg, nightText, nightBg, otherDayText, otherNightText, dayProgress, nightProgress };
}
