...
export const WindowManagerProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // 新增：为临时窗口计算错开的位置
  // 新增：为临时窗口计算错开的位置
  const getTempWindowPosition = useCallback((size = { width: 400, height: 300 }) => {
    // 获取可见窗口的数量和位置信息
    ...
    const offset = Math.min(baseOffset, Math.min(window.innerWidth, window.innerHeight) * 0.05); // 确保在小屏幕上偏移量不会太大
    ...
    // 如果最近窗口处于屏幕边缘，回到中心
    if (baseX < 10 || baseX > window.innerWidth - 50 ||
      baseY < 10 || baseY > window.innerHeight - 50) {
      baseX = centerX;
      baseY = centerY;
    }
    ...
    const checkOverlap = (testX: number, testY: number) => {
      for (const win of visibleWindows) {
        // 计算重叠区域
        const overlapX = Math.max(0, Math.min(testX + size.width, win.position.x + win.size.width) - Math.max(testX, win.position.x));
        const overlapY = Math.max(0, Math.min(testY + size.height, win.position.y + win.size.height) - Math.max(testY, win.position.y));
    ...
    if (checkOverlap(x, y)) {
      ...
      for (let i = 1; i <= 12; i++) {
        ...
        if (strategy === 0) {
          ...
          // 策略2：尝试另一个螺旋点
          const { xOffset, yOffset } = spiralOffset((positionIndex + i * 3) % 24);
          testX = baseX + xOffset;
          testY = baseY + yOffset;
        } else {
          // 策略3：基于屏幕区域分割
          const section = i % 4; // 将屏幕分为4个象限
          testX = (section % 2 === 0 ? 0.25 : 0.75) * window.innerWidth - size.width / 2;
          testY = (section < 2 ? 0.25 : 0.75) * window.innerHeight - size.height / 2;
          // 添加随机扰动
        ...
        // 确保窗口在可视区域内
        const adjustedX = Math.min(Math.max(testX, 0), window.innerWidth - size.width);
        const adjustedY = Math.min(Math.max(testY, 0), window.innerHeight - size.height);
  ...
  // 创建临时窗口 - 使用新的位置计算逻辑
  const createTempWindow = useCallback((definition: TempWindowDefinition) => {
    // 生成唯一ID
  ...
  // 更新临时窗口
  const updateTempWindow = useCallback((id: string, patch: Partial<TempWindowDefinition>) => {
    // 更新临时窗口列表
    setTempWindows(prev =>
    ...
    if (patch.showClose !== undefined) windowPatch.showClose = patch.showClose;
    if (patch.showMinimize !== undefined) windowPatch.showMinimize = patch.showMinimize;
    if (patch.showMaximize !== undefined) windowPatch.showMaximize = patch.showMaximize;
