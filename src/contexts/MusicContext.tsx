...
export function MusicProvider({ children }: MusicProviderProps) {
  ...
  const [volume, setVolume] = useState<number>(
    () => getInitialVolumeState().volume,
  );
  const [isMuted, setIsMuted] = useState<boolean>(
    () => getInitialVolumeState().isMuted,
  );
  ...
  const retrySong = useCallback(
    (originalIndex: number, retries: number) => {
  ...
  // 将原始索引转换为已加载歌曲的索引
  const mapToLoadedIndex = useCallback(
  ...
  // 将已加载歌曲的索引转换为原始索引
  const mapToOriginalIndex = useCallback(
    (loadedIndex: number): number => {
      ...
      try {
        ...
        setTracks((prev) => {
          ...
          return prev.map((track) => {
            // 查找是否有该索引的新加载结果
            const newTrack = validResults.find(
              (t) => t.originalIndex === track.originalIndex,
            );
  ...
  useEffect(() => {
    ...
    if (track && track.status === "loaded") {
      ...
      !isLoadingSongs &&
      (!resolvedSongs || resolvedSongs.length === 0)
    ) {
      setCurrentSong(null);
      setIsPlaying(false);
    }
  ...
  function onLrcLineChange(cb: LrcLineChangeCallback): () => void {
    ...
    return () => {
      lrcLineChangeCbs.current = lrcLineChangeCbs.current.filter(
        (fn) => fn !== cb,
      );
    };
  }
  ...
  const handlePrev = useCallback((): void => {
    ...
    if (playModeRef.current === "shuffle") {
      ...
      // 顺序播放
      prevLoadedIndex =
        (currentLoadedIndex - 1 + resolvedSongs.length) % resolvedSongs.length;
    }
  ...
  const handlePlaySong = useCallback(
    (loadedIndex: number): void => {
      if (
        !resolvedSongs ||
        loadedIndex < 0 ||
        loadedIndex >= resolvedSongs.length
      )
        return;
  ...
  const handleMusicCommand = useCallback(
    (subCommand: string, args: string[]): string => {
      if (!subCommand) {
        if (currentSong) {
  ...
  useEffect(() => {
    ...
    navigator.mediaSession.setActionHandler(
      "play",
      handlePlayPause as MediaSessionActionHandler,
    );
    navigator.mediaSession.setActionHandler(
      "pause",
      handlePlayPause as MediaSessionActionHandler,
    ...
    navigator.mediaSession.setActionHandler(
      "previoustrack",
      handlePrev as MediaSessionActionHandler,
    );
    navigator.mediaSession.setActionHandler(
      "nexttrack",
      handleNext as MediaSessionActionHandler,
    );
