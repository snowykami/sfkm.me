        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-blue-100/40 to-purple-100/40 dark:from-blue-500/10 dark:to-purple-500/10 rounded-lg border border-blue-200 dark:border-blue-500/20">
          ...
          <div className="flex items-center mt-2">
            ...
            >
              {t('contacts.current')}
              :
              {currentCourses.length > 0 ? currentCourses.map(course => `${course.name} (${course.begin}-${course.end})`).join('; ') : t('contacts.nocourse')}
            </span>
          ...
          <div className="mt-2">
            ...
            <ul className="mt-1">
              {todayCourses.length > 0
                ? todayCourses.sort((a, b) => a.begin.localeCompare(b.begin)).map(course => (
          ...
          <div className="mt-2">
            <span className="text-xs text-slate-500 dark:text-slate-400 italic">
              {t('contacts.tomorrowcourses')}
              {' '}
              { }
            </span>
            <ul className="mt-1">
              {tomorrowCourses.length > 0
                ? tomorrowCourses.sort((a, b) => a.begin.localeCompare(b.begin)).map(course => (
...
function CourseItem({ course, today = true }: { course: SimplifyCourse, today?: boolean }) {
  ...
  return (
    <div
      ...
      ? 'border-emerald-300'
      : isPassed
        ? 'bg-slate-300/80 dark:bg-slate-800/50 border-slate-300 dark:border-slate-600'
        : ' bg-slate-100/80 dark:bg-slate-600/50 border-slate-100 dark:border-slate-500'} 
      `}
      style={isCurrent
        ? { background: `linear-gradient(90deg, ${progressColorPassed} ${progressPercent}%, ${progressColorRemaining} ${progressPercent}%)` }
        : {
          }}
    >
      <div className="flex items-center">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            ...
            {' '}
            (
            {isCurrent ? t('contacts.durationEndClass', { duration: `${remainDuration}min` }) : isPassed ? t('contacts.ended') : t('contacts.notStarted')}
            )
          </p>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {course.location}
