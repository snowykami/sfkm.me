    ...
    es.onmessage = (event) => {
      ...
      if (data.completed !== undefined) {
        ...
        setResults((prev) => {
          ...
          let updatedResults;
          if (exists) {
            updatedResults = prev.map((hop) =>
              hop.hop === data.hop ? data : hop,
            );
          } else {
  ...
  return (
    <div className="flex flex-col h-full p-4 overflow-hidden">
      <h1 className="text-2xl font-bold mb-4">
        {t("traceroute.title", "Traceroute 工具")}
      </h1>
            ...
            <table className="w-full table-fixed">
              <thead className="sticky top-0 bg-white dark:bg-gray-800 z-10">
                <tr className="border-b dark:border-gray-600">
                  ...
                  <th className="text-left py-2 px-2 w-40">IP 地址</th>{" "}
                  {/* Adjusted width */}
                  <th className="text-left py-2 px-2 w-40">主机名</th>{" "}
                  {/* Adjusted width */}
                  <th className="text-left py-2 px-2 w-48">地理位置</th>{" "}
                  {/* New column - Added w-48 */}
                  <th className="text-left py-2 px-2 w-32">运营商</th>{" "}
                  {/* New column */}
                  ...
                  results
                    ...
                    .map((hop) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-2 text-sm truncate w-48">
                          {" "}
                          {/* Added w-48 */}
                          {hop.addr
