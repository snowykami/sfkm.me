...
export default function TerminalCommandRegister() {
  ...
  register({
    ...
    run: async (ctx: CommandArg) => {
      ...
      if (!url)
        return t('terminal.commands.curl.noUrl')
  ...
  register({
    ...
    run: async (ctx: CommandArg) => {
      ...
      if (['ls', 'list'].includes(subCommand)) {
        ...
        const id = ctx.args[2]
        if (!id)
          return t('terminal.commands.win.noId')
        const win = getWindowById(id)
        if (!win)
          return t('terminal.commands.win.notFound', { id })
        closeWindow(win.id)
  ...
  register({
    ...
    run: async (ctx: CommandArg) => {
      const id = ctx.args[1]
      if (!id)
        return t('terminal.commands.kill.noId')
      const win = getWindowById(id)
      if (!win)
        return t('terminal.commands.kill.notFound', { id })
      closeWindow(win.id)
