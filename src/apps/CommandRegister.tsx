...
import { useTerminalCommand, useTerminalCommands, useTerminalFind } from "@/contexts/TerminalCommandContext";
...
export default function TerminalCommandRegister() {
    ...
    const { resetLocalWindows, windows, closeWindow, bringToFront, updateWindow, getWindowById, openWindow } = useWindowManager();
        ...
        register({
            ...
            run: (ctx: CommandArg) => {
                ...
                if (!query || query === "help" || query === "hop") {
                    return commands.map(cmd => `${cmd.name} - ${cmd.description || t("terminal.commands.noDesc")}`).join("<br>");
                }
        ...
        register({
            ...
            run: async (ctx: CommandArg) => {
                ...
                try {
                    ...
                    const result = Function(...Object.keys(sandbox), `
                        try {
                            return ${code};
                        } catch(e) {
                    ...
                    return t("terminal.error", { error: (err instanceof Error ? err.message : String(err)) });
                }
            }
        });
        register({
            ...
            run: async () => {
                ...
                return t("terminal.commands.chtheme.result", {
                    theme: isNowDark ? t("terminal.commands.chtheme.light") : t("terminal.commands.chtheme.dark")
                });
            }
        });
        register({
            ...
            run: (ctx: CommandArg) => {
                ...
                if (!newLang) return t("terminal.commands.chlang.available", {
    ...
    }, [register, commands, mode, toggleMode, lang, setLang, resetLocalWindows, findCommand, closeWindow]);
    ...
    register({
        ...
        run: async (ctx: CommandArg) => {
            ...
            try {
                ...
                return t("terminal.commands.curl.failed", { error: (err instanceof Error ? err.message : String(err)) });
            }
        }
    });
        ...
        run: async (ctx: CommandArg) => {
            ...
            if (["ls", "list"].includes(subCommand)) {
                ...
                windows.forEach(win => {
                    ...
                    if (!win.visible) status.push(t("terminal.commands.win.statushidden"));
                    if (status.length === 0 && win.visible) status.push(t("terminal.commands.win.statusnormal"));
