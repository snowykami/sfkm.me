...
function getAdaptiveSize(
  ...
  console.log(
    `[窗口调试] 缩放系数: 宽度=${widthScale.toFixed(3)}, 高度=${heightScale.toFixed(3)}, 使用=${scale.toFixed(3)}`,
  );
  ...
  console.log(
    `[窗口调试] 可用高度: ${availableHeight}px (减去顶部栏${TOPBAR_HEIGHT}px和Dock${DOCK_HEIGHT}px)`,
  );
  ...
  if (width !== oldWidth || height !== oldHeight) {
    console.log(
      `[窗口调试] 调整以适应屏幕: ${width}x${height} (最大允许: ${maxWidth}x${maxHeight})`,
    );
  }
  ...
  if (width !== beforeMinCheck.width || height !== beforeMinCheck.height) {
    console.log(
      `[窗口调试] 调整到最小尺寸: ${width}x${height} (最小要求: ${minWidth}x${minHeight})`,
    );
  }
...
if (typeof window !== "undefined") {
  ...
  console.log(
    `[窗口调试] phoneWindowState: ${JSON.stringify(phoneWindowState.size)}`,
  );
  console.log(
    `[窗口调试] smallWindowState: ${JSON.stringify(smallWindowState.size)}`,
  );
  console.log(
    `[窗口调试] mediumWindowState: ${JSON.stringify(mediumWindowState.size)}`,
  );
  console.log(
    `[窗口调试] largeWindowState: ${JSON.stringify(largeWindowState.size)}`,
  );
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimeout = setTimeout(() => {
      ...
      console.log(
        `[窗口调试] phoneWindowState: ${JSON.stringify(oldPhoneSize)} -> ${JSON.stringify(phoneWindowState.size)}`,
      );
      console.log(
        `[窗口调试] smallWindowState: ${JSON.stringify(oldSmallSize)} -> ${JSON.stringify(smallWindowState.size)}`,
      );
      console.log(
        `[窗口调试] mediumWindowState: ${JSON.stringify(oldMediumSize)} -> ${JSON.stringify(mediumWindowState.size)}`,
      );
      console.log(
        `[窗口调试] largeWindowState: ${JSON.stringify(oldLargeSize)} -> ${JSON.stringify(largeWindowState.size)}`,
      );
    }, 200); // 200ms 防抖延迟
  });
}
